...
select texts,
 (len(texts) -len(replace(texts, ' ', ''))) as spacecount from CountSpaces
...
Select *,
Case when TerminationDate is not null then DATEDIFF(month, HIRE_DATE, GETDATE())
	else DATEDIFF(month, HIRE_DATE, GETDATE())
end
from Employees
...
Select *, 
LEFT(VALS, CHARINDEX(' ', VALS + ' ') -1) FROM GetIntegers
...
dECLARE @pos INT = PATINDEX('%[0-9]%', @str);
SELECT 
    @str AS OriginalString,
    SUBSTRING(@str, 1, @pos - 1) AS CharacterPart,
    SUBSTRING(@str, @pos, LEN(@str)) AS IntegerPart;
