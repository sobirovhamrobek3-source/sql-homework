...
select REPLACE( phone_number, '124', '999') as new_phone from Employees
...
select manager_Id as total_salary,
...
SELECT *
FROM SingleOrder
ORDER BY case when id = 0 then 1 else 0
end , id

select * ,
coalesce(id, ssn, passportid, itin) from person
...
Select concat(0.9*100, '%')

select AVG(department_ID) from Employees

select DEPARTMENT_ID,
   count(*) as totalemployees,
   100.0 * sum(case when DATEDIFF(YEAR, HIRE_DATE,
   GETDATE()) >3 
...
select studentID,
fullname, 
sum(marks) over(order by studentID) from Students
...
select playerA,
       playerB,
	   sum(score ) from  playerscores
	   group by playerA, playerB



SELECT
  STRING_AGG(CASE WHEN SUBSTRING(value, 1, 1) COLLATE Latin1_General_BIN LIKE '[A-Z]' THEN value END, '') AS UppercaseLetters,
  STRING_AGG(CASE WHEN SUBSTRING(value, 1, 1) COLLATE Latin1_General_BIN LIKE '[a-z]' THEN value END, '') AS LowercaseLetters,
  STRING_AGG(CASE WHEN SUBSTRING(value, 1, 1) LIKE '[0-9]' THEN value END, '') AS Numbers,
  STRING_AGG(CASE WHEN SUBSTRING(value, 1, 1) COLLATE Latin1_General_BIN NOT LIKE '[A-Za-z0-9]' THEN value END, '') AS OtherCharacters
